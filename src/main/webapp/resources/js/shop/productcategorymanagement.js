$(function() {
	...
	$('#submit').click(function() {
		var tempArr = $('.temp'); // get all rows newly created
		var productCategoryList = [];
		tempArr.map(function(index, item) {
			var tempObj = {};
			tempObj.productCategoryName = $(item).find('.category').val();
			tempObj.priority = $(item).find('.priority').val();
			// don't append shop id here because the controller has got shop id from session
		...
		$.ajax({
			...
			success : function(data) {
				if (data.success) {
					...
					getList(); // to render updated list sorted by priority
				} else {
					$.toast('submit failed');
				}
			}
